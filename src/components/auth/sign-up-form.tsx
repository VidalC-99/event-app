...
export function SignUpForm() {
  ...
  const passwordStrength = useMemo(() => {
    ...
    const width = `${((idx + 1) / 6) * 100}%`
    return { label: labels[idx], color: colors[idx], width }
  }, [password])
  ...
  const signUpWithGoogle = async () => {
    ...
    try {
      ...
      // En cas de succès, l'utilisateur sera redirigé par Supabase.
    } catch (e: any) {
      setError(e?.message || "Une erreur est survenue lors de l'inscription avec Google")
    } finally {
      // S'assure de réactiver le bouton si la redirection n'a pas lieu
      ...
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
          ...
          <div className="space-y-2">
            ...
            {!errors.email && (
              <p id="email-help" className="text-xs text-muted-foreground">
                Nous enverrons un email de confirmation pour activer votre compte.
              </p>
            )}
          ...
          <div className="space-y-2">
            ...
            <div className="mt-1">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Robustesse: {password ? passwordStrength.label : "—"}
              </p>
            </div>
          </div>
          ...
          <div className="text-center text-sm">
            Déjà un compte ?{" "}
            <Link href="/src/app/auth/login" className="text-primary hover:underline">
              Se connecter
            </Link>
          </div>
        </form>
